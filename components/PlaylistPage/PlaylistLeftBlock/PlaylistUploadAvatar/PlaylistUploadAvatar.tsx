...
const PlaylistUploadAvatar: React.FC<Props> = ({ isPlaylistHasImg,playlistId,updatePlaylistData }) => {
  ...
  return (
    <div className={s.playlistUploadAvatar}>
      <div style={{ position: 'relative' }}>
        ...
        />
        {isPopupOpen && <SettingsBtnPopUp   styles={{ top: '100%', left: '0' }}
            closePopup={() => setIsPopupOpen(false)}>
            <UploadPlaylistImg playlistId={playlistId} closePopup={()=>{
     updatePlaylistData()
   setIsPopupOpen(false)
            }}/>
            <DeletePlaylistImg playlistId={playlistId} closePopup={()=>{
              updatePlaylistData()
            setIsPopupOpen(false)
            }} />
            </SettingsBtnPopUp>}
