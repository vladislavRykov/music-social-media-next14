...
const LibraryMain = () => {
  ...
  const newPlaylists:PlaylistType[] | undefined = moveToFront(playlists?.data||[],findPlaylistById('favorites'))

  return (
    <div className={s.librarymain}>
      <div className={s.itemList}>
        {status==='success' && newPlaylists.map((playlist) => {
          if (playlist.access_type === AccessType.Private && playlist.userId._id !== currentUserId)
            return null;
          return (
            <LibPlaylist
            type={playlist.type}
