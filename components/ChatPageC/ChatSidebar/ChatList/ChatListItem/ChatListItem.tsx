...
import chatImgMock from '@/public/avatar.jpg'
...
type Props ={
    lastMessage: {from:{userId: string,username: string} ,type:string,message?: string,time: Date} |null,
...
const ChatListItem = ({lastMessage,type,chatName,chatImg,chatId,selectedChat}:Props) => {
  ...
  return (
    ...
    <Link href={`/chat/${chatId}`} onClick={handleClick} ref={containerRef} className={cn(s.chatListItem,{[s.chatListItem_selected]:selectedChat===chatId})}>
        <Image className={s.chatListItem_img} src={chatImg||chatImgMock} alt='chat img' height={50} width={50} />
        <div  className={s.chatListItem_rightBlock}>
            <div  className={s.chatListItem_nameAndTime}>
            <h2  className={s.chatListItem_chatname} style={selectedChat===chatId ?{color: '#fafafa'}:{}}>{chatName}</h2>
     ...
     {isPopupOpen && <ChatPopup styles={{top: blockPosition.y,left: blockPosition.x,position: 'fixed'}} closePopup={()=>setIsPopupOpen(false)}>
