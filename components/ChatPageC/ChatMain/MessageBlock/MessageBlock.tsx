...
const MessageBlock = ({chatId,addNewMessage}:Props) => {
  ...
  return (
    <div className={s.messageBlock}>
        <div className={s.messageBlock_content}>
          ...
          <EmojiPickerBlock addEmojiToMessage={(emoji:string)=>setMessageValue(message=>message+emoji)}/>
          <textarea className={s.messageBlock_textarea} rows={1} onChange={onChange} value={messageValue}/>
...
{/* 
          ...
          shouldReturn
           onChange={setMessageValue}
            ...
            { isTimerActive && <div className={s.messageBlock_recordingTimer}>
               <span>
                 {secondsToStringTimer(recordingTime)}
                </span>
         ...
         {messageValue ? <div className={s.messageBlock_defBtn} onClick={sendMessageHandler}>
           <IoMdSend className={s.messageBlock_iconBtn+ " "+s.messageBlock_sendIcon}  size={22}/>
          </div>:
          <AudioRecorder currentUserData={currentUserData} addNewMessage={addNewMessage} startTimer={startTimer}stopTimer={stopTimer} chatId={chatId}/>}
        </div>
    </div>
  )
}
