...
const EventList = ({isOnlyFreeEvents}:Props) => {
  ...
  const getEvents = async(locationData:Location|null,isOnlyFree:boolean,nextPageUrl?: string|null)=>{
    let page = null;
    ...
    const res = await fetch(
      `http://localhost:3000/api/events?is_free=${isOnlyFree}&location=${locationData.slug}&actual_since=${actual_since || date.toISOString()}&page=${page||1}`
    )
  ...
  const refObserver = useScrollPagination({
    ...
    scrollDeps: [location?.slug,isOnlyFreeEvents,eventData?.next]
  });
  useEffect(() => {
