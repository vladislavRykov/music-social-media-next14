...
import { createChat, findAllCurrentUserChats, findChatById, findDialogByMembers } from '@/dal/chat';
import { createAudioMessage, createMessage, deleteMessageById, findMessagesByChatId } from '@/dal/message';
...
export const findMessagesByChatIdAction = async (chatId:string) => {
  try {
    ...
    if(chat.members.filter((member)=>member.toString() === session.userId).length===0) return { ok: false, data: null, message: 'Вы не участник этого чата' };

    const messages =await findMessagesByChatId<Overwrite<MessageMongoose,{author: UserProfileData}>[]>(chatId,[{path:'author',select: '_id username avatar banner'}])
